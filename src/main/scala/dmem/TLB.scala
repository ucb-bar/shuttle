...
class ShuttleDTLB(lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new ShuttleDTLBReq(lgMaxSize)))
    val resp = Output(new ShuttleDTLBResp)
  ...
  val mpu_ppn = Mux(do_refill, refill_ppn, io.req.bits.vaddr >> pgIdxBits)
